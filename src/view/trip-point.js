...
function createTripPoint (point, destinations, offers) {
  ...
  <li class="trip-events__item">
    <div class="event">
      <time class="event__date" datetime="${humanizeEventFullDate(dateFrom)}">${humanizeEventCustomDate(dateFrom)}</time>
      <div class="event__type">
        <img class="event__type-icon" width="42" height="42" src="img/icons/${type.toLowerCase()}.png" alt="Event type icon">
      </div>
      <h3 class="event__title">${capitalizeFirstLetter(type)} ${capitalizeFirstLetter(pointDestination.name)}</h3>
      <div class="event__schedule">
        <p class="event__time">
          <time class="event__start-time" datetime="${humanizeEventEditDate(dateFrom)}">${humanizeEventTimeDate(dateFrom)}</time>
          &mdash;
          <time class="event__end-time" datetime="${humanizeEventEditDate(dateTo)}">${humanizeEventTimeDate(dateTo)}</time>
        </p>
